...
import type { Loki, LokiResultset, LokiQueryObject } from '../type'
...
function performJoin(join: LokiJoin, loki: Loki): DirtyRaw[] {
  ...
  const records = collection.find(query as LokiQueryObject | undefined).data()

  return records as DirtyRaw[]
}
...
function performQuery(query: SerializedQuery, loki: Loki): LokiResultset {
  ...
  let resultset = collection.find(mainQuery as LokiQueryObject | undefined)
...
export function executeQuery(query: SerializedQuery, loki: Loki): DirtyRaw[] {
  ...
  if (lokiTransform) {
    return lokiTransform(results, loki) as DirtyRaw[]
  }

  return results as DirtyRaw[]
}
