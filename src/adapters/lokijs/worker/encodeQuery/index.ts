...
export type LokiRawQueryObject = {
  [key: string]: unknown
}
export type LokiRawQuery = LokiRawQueryObject | undefined
type LokiOperator =
  | '$aeq'
  | '$eq'
  | '$gt'
...
const encodeComparison = (comparison: Comparison, value: unknown): LokiRawQueryObject => {
  ...
  const right = comparison.right as ComparisonRightFields
...
const columnCompRequiresColumnNotNull: { [K in Operator]?: boolean | undefined } = {
  gt: true,
  gte: true,
  lt: true,
  lte: true,
...
const encodeWhereDescription = ({ left, comparison }: WhereDescription): LokiRawQuery => {
  const { operator, right } = comparison
  const col: string = left
  const rightFields = right as ComparisonRightFields
  // NOTE: order of ||s is important here, since .value can be falsy, but .column and .values are either truthy or are undefined
  const comparisonRight = rightFields.column || rightFields.values || rightFields.value

  if (typeof rightFields.value === 'string') {
  ...
  const colName = rightFields.column
...
const encodeCondition =
  ...
  (clause: Where): LokiRawQuery => {
    switch (clause.type) {
      ...
      case 'loki':
        return clause.expr as LokiRawQuery
      default:
        throw new Error(`Unknown clause ${clause.type}`)
    }
  }

const encodeConditions = (associations: QueryAssociation[], conditions: Where[]): LokiRawQuery[] =>
...
const encodeAnd: (associations: QueryAssociation[], clause: And) => LokiRawQuery =
  encodeAndOr('$and')
const encodeOr: (associations: QueryAssociation[], clause: Or) => LokiRawQuery = encodeAndOr('$or')
...
const encodeRootConditions = (associations: QueryAssociation[], conditions: Where[]): LokiRawQuery =>
