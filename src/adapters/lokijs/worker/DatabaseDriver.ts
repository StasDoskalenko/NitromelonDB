...
import type { Loki, LokiCollection, LokiAdapterOptions, LokiRawDocument } from '../type'
...
type LocalStorageDoc = LokiRawDocument & {
  key?: unknown
  value?: unknown
}
...
export default class DatabaseDriver {
  ...
  cachedRecords: Map<TableName, Set<RecordId>> = new Map()
  ...
  isCached(table: TableName, id: RecordId): boolean {
    const cachedSet = this.cachedRecords.get(table)
    return cachedSet ? cachedSet.has(id) : false
  }

  markAsCached(table: TableName, id: RecordId): void {
    const cachedSet = this.cachedRecords.get(table)
    if (cachedSet) {
      cachedSet.add(id)
    } else {
  ...
  removeFromCache(table: TableName, id: RecordId): void {
    const cachedSet = this.cachedRecords.get(table)
    if (cachedSet) {
      cachedSet.delete(id)
    }
  ...
  getCache(table: TableName): Set<RecordId> {
    const cache = this.cachedRecords.get(table)
    if (cache) {
      return cache
    }

    const newCache = new Set<RecordId>([])
    this.cachedRecords.set(table, newCache)
    return newCache
  }

  find(table: TableName, id: RecordId): CachedFindResult {
  ...
  queryIds(query: SerializedQuery): RecordId[] {
    return executeQuery(query, this.loki).map((record) => record.id as RecordId)
  }

  unsafeQueryRaw(query: SerializedQuery): unknown[] {
  ...
  batch(operations: BatchOperation[]): void {
    ...
    try {
      const recordsToCreate: { [table: TableName]: RawRecord[] | undefined } = {}
      ...
      Object.entries(recordsToCreate).forEach(([table, raws]) => {
        ...
        raws.forEach((rawRecord) => {
          cache.add(rawRecord.id)
        })
      })
      ...
      operations.forEach((operation) => {
        ...
        switch (type) {
          case 'update': {
            ...
            const existing = this.loki.getCollection(table).by('id', rawOrId.id) as LokiRawDocument
            const lokiId = existing.$loki
      ...
      this._fatalError(error as Error)
    }
  }
  ...
  getDeletedRecords(table: TableName): RecordId[] {
    return this.loki
      ...
      .map((record) => record.id as RecordId)
  }
  ...
  unsafeExecute(operations: UnsafeExecuteOperations): void {
    ...
    const lokiBlock = (operations as { loki: (loki: Loki) => void }).loki
    lokiBlock(this.loki)
  }
  ...
  _addCollection(tableSchema: TableSchema): void {
    ...
    const indexedColumns = columnArray.reduce<string[]>(
      (indexes, column) => (column.isIndexed ? indexes.concat([column.name]) : indexes),
      [],
    )
  ...
  set _databaseVersion(version: SchemaVersion) {
  ...
  async _migrate(steps: MigrationStep[]): Promise<void> {
    steps.forEach((step) => {
      if (step.type === 'create_table') {
        ...
        const unsupported: { type: string } = step
        throw new Error(`Unsupported migration step ${unsupported.type}`)
      }
    })
  ...
  _executeAddColumnsMigration({ table, columns }: AddColumnsMigrationStep): void {
    ...
    collection.findAndUpdate({}, (record) => {
      columns.forEach((column) => {
        setRawSanitized(record as RawRecord, column.name, null, column)
      })
    })
  ...
  _compactQueryResults(records: DirtyRaw[], table: TableName): CachedQueryResult {
    const cache = this.getCache(table)
    return records.map((raw) => {
      const id = raw.id as RecordId
  ...
  _findLocal(key: string): LocalStorageDoc | undefined {
    const localStorage = this._localStorage
    return localStorage && (localStorage.by('key', key) as LocalStorageDoc | undefined)
  }
