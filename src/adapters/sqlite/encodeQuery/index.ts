...
import type { SQL, SQLiteArg } from '../type'

function mapJoin<T>(array: T[], mapper: (item: T) => string, joiner: string): string {
...
const encodeValues = (values: NonNullValues): string =>
  `(${mapJoin(values as unknown[], encodeValue, ', ')})`

const getComparisonRight = (table: TableName, comparisonRight: ComparisonRight): string => {
  if ('values' in comparisonRight) {
    return encodeValues(comparisonRight.values)
  } else if ('column' in comparisonRight) {
...
const operators: { [K in Exclude<Operator, 'includes'>]: string } = {
  eq: 'is',
  notEq: 'is not',
  gt: '>',
  gte: '>=',
...
const encodeComparison = (table: TableName, comparison: Comparison): string => {
  const { operator } = comparison
  if (operator === 'between') {
    const { right } = comparison
    return 'values' in right
      ? `between ${encodeValue(right.values[0])} and ${encodeValue(right.values[1])}`
      : ''
  }

  return `${operators[operator as Exclude<Operator, 'includes'>]} ${getComparisonRight(table, comparison.right)}`
}

const encodeWhere =
  (table: TableName, associations: QueryAssociation[]) =>
  (where: Where): string => {
    switch (where.type) {
      case 'and':
...
const encodeWhereCondition = (
  ...
  if (operator === 'weakGt' && 'column' in comparison.right) {
    return encodeWhere(
      table,
      associations,
    )(
...
const encodeMethod = (table: TableName, countMode: boolean, needsDistinct: boolean): string => {
  if (countMode) {
...
const encodeOrderBy = (table: TableName, sortBys: SortBy[]): string => {
  if (sortBys.length === 0) {
    return ''
  }
  const orderBys = sortBys
    .map((sortBy) => {
      if ('sortExpr' in sortBy) {
...
const encodeLimitOffset = (limit?: number | undefined, offset?: number | undefined): string => {
  if (!limit) {
    return ''
  }
