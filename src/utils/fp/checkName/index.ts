...
export default function checkName<T extends string | TableName<any> | ColumnName>(name: T): T {
  ...
  invariant(!['__proto__', 'constructor', 'prototype', 'hasOwnProperty', 'isPrototypeOf', 'toString', 'toLocaleString', 'valueOf'].includes(name), `Unsafe name '${name}' not allowed (Object prototype property)`);
  invariant(name.toLowerCase() !== '$loki', `Unsafe name '${name}' not allowed (reserved for LokiJS compatibility)`);
  invariant(!['rowid', 'oid', '_rowid_', 'sqlite_master'].includes(name.toLowerCase()), `Unsafe name '${name}' not allowed (reserved for SQLite compatibility)`);
  invariant(!name.toLowerCase().startsWith('sqlite_stat'), `Unsafe name '${name}' not allowed (reserved for SQLite compatibility)`);
  invariant(!name.startsWith('__'), `Unsafe name '${name}' not allowed (names starting with '__' are reserved for internal purposes)`);
  invariant(safeNameCharacters.test(name), `Unsafe name '${name}' not allowed (names must contain only safe characters ${safeNameCharacters.toString()})`);
