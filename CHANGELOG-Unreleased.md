...
- `useWriter(model, writer)` hook: a stable callback that runs `writer` inside `model.database.write()`, with `writer`'s first argument typed as the concrete `Model` subclass passed in. Tracks `isPending`/`error` for you, and doesn't require `writer` to be memoized (the latest one passed in is always the one that runs) — see [docs](https://stasdoskalenko.github.io/NitromelonDB/docs/Hooks#usewriter--writing-anchored-to-a-record).
