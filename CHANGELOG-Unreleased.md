...
- NotesApp Windows e2e: `ScrollView` of page-sized cards so new rows stay in the UIA tree. Add note types via SendKeys into an uncontrolled Windows title field (WinAppDriver `keys`/`setValue` drop lowercase). `textVisible` requires `isDisplayed()`. Windows list keeps off-tree UIA anchors for titles/pin/delete.
