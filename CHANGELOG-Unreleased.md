...
- `useAtomicWriter(collection, record, builder)` hook: the narrower sibling of `useWriter` for the single most common write — pass a `record` and it updates it, leave it out (or pass `null`/`undefined`) and it creates a new one in `collection` instead. No free-form writer function, just the same field-setting `builder` `.create()`/`.update()` already take, which keeps the Writer to nothing but field assignments (no risk of slow/unrelated logic stalling every other write in the app) — see [docs](https://stasdoskalenko.github.io/NitromelonDB/docs/Hooks#useatomicwriter--updating-or-creating-one-record-nothing-else).
