...
public:
    ...
    // Sizes the prepared-statement cache for the current schema (see StatementCache). Called once
    // the schema is known: after initialize() finds an up-to-date database, and at the end of
    // unsafeResetDatabase() and migrate().
    void fitStatementCacheToSchema();
...
private:
    ...
    StatementCache cachedStatements_; // bounded, LRU-evicting -- see StatementCache.h
    ...
    void fitStatementCacheToSchemaLocked();
