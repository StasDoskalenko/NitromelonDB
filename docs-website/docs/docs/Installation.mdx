...
NitromelonDB fully supports Expo **development builds**, **EAS Build**, and **EAS Update**. Native SQLite is not available in Expo Go.
...
2. Add the config plugin to `app.json` or `app.config.js`:
   ...
   The plugin runs during `expo prebuild` (EAS Build does this for you). It writes Android ProGuard keep rules so release / EAS production minify cannot strip the Nitro JNI loader. It does **not** install the old `watermelondb-jsi` Android module — SQLite goes through Nitro autolinking.
...
The Android AAR also ships this (plus a keep for `com.margelo.nitro.watermelondb.**`) via `consumer-rules.pro`. The Expo config plugin writes the same rules during prebuild.
