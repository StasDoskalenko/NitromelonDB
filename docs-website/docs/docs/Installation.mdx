...
2. Add the config plugin to `app.json` or `app.config.js`:
   ...
   The plugin runs during `expo prebuild` (EAS Build does this for you). It does **not** install the old `watermelondb-jsi` Android module — SQLite goes through Nitro autolinking.
