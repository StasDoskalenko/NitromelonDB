...
Native SQLite on iOS and Android uses [Nitro Modules](https://nitro.margelo.com) and React Native **autolinking** (`react-native.config.js` + the `NitromelonDB` podspec). `simdjson` is compiled into that pod from vendored sources. iOS pulls CocoaPods `FMDB` (system sqlite3) for the ObjC helper path. You do not add CocoaPods or Gradle lines by hand, and you do not run a separate setup CLI.
...
`run-ios` installs pods and applies autolinking. You do not add `pod 'NitromelonDB'`, `pod 'simdjson'`, or `pod 'FMDB'` to the Podfile.
