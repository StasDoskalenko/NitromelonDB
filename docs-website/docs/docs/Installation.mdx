...
On web, the normal `nitromelondb/adapters/sqlite` import selects wa-sqlite automatically. SQLite runs in a dedicated worker and persists its database in IndexedDB through `IDBBatchAtomicVFS`. There is no LokiJS or in-memory fallback, and no COOP/COEP header requirement.

Expo SDK 57 web workers require bundle splitting. Expo Router already provides the runtime; projects without Router should install `@expo/metro-runtime` and load it through a web-only platform module:

```ts
// webRuntime.web.ts
import '@expo/metro-runtime'

// webRuntime.ts (native) is intentionally empty
// index.ts imports './webRuntime'
```

If you have a custom `metro.config.js`, preserve Expo's defaults and ensure WASM is an asset:
...
The package resolves its worker and WASM automatically. A custom host or worker policy can override them:
...
Run `npx expo export -p web` before release and confirm the output contains a worker bundle and the `.wasm` asset. `usesExclusiveLocking` is unsupported on web. SSR may import and construct the adapter, but server-side database operations are rejected; use the authoritative server database there and let the hydrated browser open its own replica. Existing LokiJS databases are not migrated.

For the runtime design, transaction behavior, offline and multi-tab semantics, known limitations,
testing strategy, and future work, see the [wa-sqlite web adapter](./Implementation/WaSQLiteAdapter.md).
...
<details>
  <summary>Why does <code>Podfile.lock</code> list <code>React-Core</code>, <code>React-jsi</code>, <code>ReactCommon</code> and other React pods under <code>NitromelonDB</code>?</summary>

This is expected, and it is not specific to NitromelonDB — every React Native native module that uses the New Architecture (JSI / TurboModules / Fabric) produces the same entries. `react-native-reanimated`, `react-native-screens`, `react-native-gesture-handler`, `react-native-mmkv` and WatermelonDB itself all list a dozen-plus `React-*` pods in a consumer's `Podfile.lock`.

The `NitromelonDB` podspec declares `React-Core`, `React-jsi` and `React-callinvoker`, then calls React Native's own `install_modules_dependencies` helper, which expands to the full New-Arch dependency set (`React-Fabric`, `React-graphics`, `React-ImageManager`, `RCTRequired`, `RCTTypeSafety`, `ReactCommon/turbomodule/core`, `ReactCodegen`, `Yoga`, `hermes-engine`, …). CocoaPods records those edges in `Podfile.lock`.

It does **not** mean:

- **Duplicate React is compiled.** CocoaPods deduplicates — your app has one `React-Core` pod and every module links against that same one.
- **NitromelonDB pins a React version.** `install_modules_dependencies` inherits whatever version your app's `use_react_native!` configured; the podspec adds no version constraints on any `React-*` pod.
