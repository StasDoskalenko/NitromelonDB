...
`react-native-nitro-modules` `>=0.35.2` is required for SQLite on iOS and Android. It is optional on web / Node / Electron. Do not add it a second time if it is already in the app.
...
Windows New Architecture (RNW 0.84 / RN 0.84.1) is the [`examples/NotesApp_windows`](https://github.com/StasDoskalenko/NitromelonDB/tree/master/examples/NotesApp_windows) playground. Native SQLite for that stack is still being ported; the old UWP Paper module in `native/windows` does not autolink into WinAppSDK.
