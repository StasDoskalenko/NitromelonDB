...
- `useDatabaseReady(database)` (`nitromelondb/hooks` or `nitromelondb/react`) is now documented in [Hooks](https://stasdoskalenko.github.io/NitromelonDB/docs/Hooks#usedatabaseready--gating-your-own-ui-on-readiness) alongside the other hooks (it shipped in an earlier Unreleased entry above but was missing from the hooks guide).
