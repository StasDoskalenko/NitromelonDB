...
Expo SDK 57 development-build app that opens SQLite through `SQLiteAdapter` → a typed `NitromelonDatabase` HybridObject. This will not run in Expo Go. `app.json` includes the `nitromelondb` config plugin so EAS Build / `expo prebuild` apply ProGuard keep rules (Nitro autolinking covers the rest; the old Android JSI Gradle module is not used).
